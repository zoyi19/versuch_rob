...
class config:
    class common:
        ...
        walk_pos_threshold = 0.17  # 走路事件的位置阈值，单位米
    ...
    class pick:
        """搬框配置"""
        tag_id = [2,11]  # 搬箱的tag码的id，可以是单个数字，也可以是列表，，列表大小与grab_box_num相同。例如：tag_id=1，或者tag_id=[1, 2, 3, 4, 5]
        step_back_distance = 0.0
        ...
        stand_in_tag_pos = (0.0, 0.0, 0.42)  # 站立位置在目标标签中的位置猜测，单位米
        ...
        box_behind_tag = 0.18  # 箱子在tag后面的距离，单位米
        box_beneath_tag = 0.08  # 箱子在tag下方的距离，单位米
        box_left_tag = -0.0  # 箱子在tag左侧的距离，单位米

        force_ratio_z = 0.0
        lateral_force = 0.0  # 侧向夹持力，单位N
        waist_degree = -0.0  # 拿箱子后转腰角度，单位度
        arm_total_time = 4.0  # 拿箱子时手臂运动总时间，单位秒
    ...
    class place:
        ...
        stand_in_tag_pos = (-0.1, 0.0, 0.88)  # 放置位置站立位置在目标标签中的位置猜测，单位米
        ...
        box_behind_tag = -0.4  # 箱子在tag后面的距离，单位米
        box_beneath_tag = 0.55  # 箱子在tag下方的距离，单位米
        box_left_tag = 0.1  # 箱子在tag左侧的距离，单位米

        force_ratio_z = 0.0
        lateral_force = 0.0  # 侧向夹持力，单位N
        waist_degree = 180.0  # 放箱子后转腰角度，单位度
        arm_total_time = 2.0  # 放箱子时手臂运动总时间，单位秒