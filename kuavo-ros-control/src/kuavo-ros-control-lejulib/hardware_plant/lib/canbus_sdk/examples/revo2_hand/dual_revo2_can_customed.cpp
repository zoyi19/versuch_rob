/*
 * dual_revo2_can_customed.cpp
 *
 * 双手Revo2灵巧手CAN通信示例程序
 *
 * 功能：
 * - 同时控制左右手两个Revo2灵巧手
 * - 左手使用can0总线，右手使用can1总线
 * - 支持手指位置控制和持续时间设置
 * - 实时显示电机状态信息
 * - 通过消息队列处理CAN通信数据
 *
...
#define RHAND_DEVICE_ID 0x02  /* 右手 */
...
CanMessageQueue g_left_hand_message_queue;
CanMessageQueue g_right_hand_message_queue;
...
int main(int argc, char const *argv[])
{
  ...
  result = canbus_controller->openCanBus("can1", CanBusModelType::LEJU_CAN_B, bitrate);
  if (!result.has_value()) {
      printf("Failed to open CAN bus: %s\n", errorToString(result.error()));
      return -1;
  }
  ...
  canbus_controller->registerDevice(canbus_sdk::DeviceInfo{
      .device_name = "revo2_Lhand",
      .device_type = canbus_sdk::DeviceType::REVO2_HAND,
      .device_id = LHAND_DEVICE_ID,
      .matcher = Matcher
  }, "can0", callback_params);
  canbus_controller->registerDevice(canbus_sdk::DeviceInfo{
      .device_name = "revo2_Rhand",
      .device_type = canbus_sdk::DeviceType::REVO2_HAND,
      .device_id = RHAND_DEVICE_ID,
      .matcher = Matcher
  }, "can1", callback_params);
  ///////////////////////////////////////////////////////////////////////////////
  ...
  auto r_handle_right = create_device_handler();
  auto l_handle_right = create_device_handler();
  get_device_info(l_handle_right, LHAND_DEVICE_ID);
  get_device_info(r_handle_right, RHAND_DEVICE_ID);
  ...
  for(int j = 0; j < 10; ++j) {
    for (int i = 0; i < num_steps; ++i) {
      stark_set_finger_positions_and_durations(r_handle_right, RHAND_DEVICE_ID,
                                             positions_list[i], durations, 6);
      stark_set_finger_positions_and_durations(l_handle_right, LHAND_DEVICE_ID,
                                              positions_list[i], durations, 6);
      print_motor_status(r_handle_right, RHAND_DEVICE_ID);
      print_motor_status(l_handle_right, LHAND_DEVICE_ID);
      usleep(delay);
    }
...
void setup_canfd_callbacks()
{
  ...
  set_can_tx_callback([](uint8_t slave_id,
                         ...
                         uintptr_t data_len) -> int
                      {
                        ...
                        // 根据slave_id选择对应的CAN总线发送
                        if (slave_id == LHAND_DEVICE_ID) {
                          canbus_controller->sendMessage("can0", can_frame);
                        } else if (slave_id == RHAND_DEVICE_ID) {
                          canbus_controller->sendMessage("can1", can_frame);
                        } else {
                            printf("Unknown slave_id: %d for TX\n", slave_id);
                            return -1;
                        }
  ...
  set_can_rx_callback([](uint8_t slave_id,
                      ...
                      {
                        // 根据slave_id选择对应的消息队列
                        CanMessageQueue *target_queue = nullptr;
                        if (slave_id == LHAND_DEVICE_ID) {
                          target_queue = &g_left_hand_message_queue;
                        } else if (slave_id == RHAND_DEVICE_ID) {
                          target_queue = &g_right_hand_message_queue;
                        } else {
                          printf("Unknown slave_id: %d\n", slave_id);
                          return -1;
                        }

                        // 从对应的消息队列中读取数据
                        canbus_sdk::CanMessageFrame first_frame;
                        if (!target_queue->try_pop(&first_frame)) {
                          printf("Failed to read from CAN message queue for slave_id: %d\n", slave_id);
                          return -1;
                        }
                        ...
                        if (base_cmd == 0x0B) {
                            while (frame_count < MAX_BUF_SIZE) {
                                ...
                                if (!target_queue->front(&peek_frame))
                                    break;
                                ...
                                if (!target_queue->try_pop(&next_frame))
                                    break;
...
void can_receive_callback(canbus_sdk::CanMessageFrame* frame, const canbus_sdk::CallbackContext* context)
{
  ...
  // 解析CAN ID中的设备ID，将消息存入对应的队列
  uint32_t device_id = (frame->id.SID >> 7) & 0x0F;
  if (device_id == LHAND_DEVICE_ID) {
    g_left_hand_message_queue.push(*frame);
  } else if (device_id == RHAND_DEVICE_ID) {
    g_right_hand_message_queue.push(*frame);
  }
