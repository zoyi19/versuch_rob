...
public:
    Revo1CanTest() : is_initialized_(false) {}
    ...
    bool init_can_buses() {
        ...
        if (!parser.parseFromFile(config_file)) {
            printf("\033[31m[Revo1CanTest] ERROR: 解析配置文件失败: %s\033[0m\n", config_file.c_str());
            return false;
        }
        ...
        if (canbus_configs_.empty()) {
            printf("\033[31m[Revo1CanTest] ERROR: 未找到CAN总线配置\033[0m\n");
            return false;
        }
        ...
        for (const auto& canbus_config : canbus_configs_) {
            // 先检查该CAN总线上是否有非忽略的REVO1设备
            auto devices = parser.getDevices(canbus_config.name, DeviceType::REVO1_HAND);
            bool has_valid_device = std::any_of(devices.begin(), devices.end(),
                [](const auto& device) { return !device.ignore; });

            if (!has_valid_device) {
                printf("\033[33m[Revo1CanTest] WARN: CAN总线 %s 上没有有效的REVO1设备，跳过\033[0m\n", canbus_config.name.c_str());
            ...
            if (!result.has_value()) {
                printf("\033[33m[Revo1CanTest] WARN: 打开CAN总线 %s 失败: %s，跳过该总线上的设备\033[0m\n",
                       canbus_config.name.c_str(), errorToString(result.error()));
                continue; // 跳过这个失败的CAN总线
            }

            printf("\033[32m[Revo1CanTest] INFO: 成功打开CAN总线: %s\033[0m\n", canbus_config.name.c_str());

            // 保存该CAN总线上的所有非忽略设备
            for (const auto& device : devices) {
                if (device.ignore) {
                    printf("\033[33m[Revo1CanTest] WARN: 设备 %s 被忽略，跳过\033[0m\n", device.name.c_str());
                    continue;
                }
                device_configs_.push_back(device);
                printf("\033[36m[Revo1CanTest] INFO: 在总线 %s 上找到设备: %s\033[0m\n",
    ...
    // 连接REVO1设备
    bool connect_revo1_devices() {
        printf("\033[36m[Revo1CanTest] INFO: 连接REVO1设备...\033[0m\n");

        if (device_configs_.empty()) {
            printf("\033[31m[Revo1CanTest] ERROR: 未找到REVO1设备 - 请先调用init_can_buses()!\033[0m\n");
            return false;
        }

        // 显示找到的设备
        for (const auto& device_config : device_configs_) {
            printf("\033[36m[Revo1CanTest] INFO: 找到设备: %s\033[0m\n", device_config.name.c_str());
        }
        ...
        hands_.clear();
        for (const auto& device_config : device_configs_) {
            auto hand = Revo1CanDexhand::Connect(device_config);
            if (hand) {
                hands_.push_back(std::move(hand));
                printf("\033[32m[Revo1CanTest] INFO: 设备 %s 连接成功\033[0m\n", device_config.name.c_str());
                connected_count++;
            } else {
                printf("\033[31m[Revo1CanTest] ERROR: 设备 %s 连接失败\033[0m\n", device_config.name.c_str());
            }
        }

        printf("\033[36m[Revo1CanTest] INFO: 连接成功 %d/%d 个设备\033[0m\n", connected_count, static_cast<int>(device_configs_.size()));
        ...
        for (size_t i = 0; i < hands_.size(); ++i) {
            if (hands_[i]) {
                ...
                if (hands_[i]->getDeviceInfo(dev_info)) {
                    std::cout << "\033[36m[Revo1CanTest] INFO: Device " << dev_info << "\033[0m" << std::endl;
                }
            }
        }
    ...
    void setup_grip_force() {
        ...
        for (size_t i = 0; i < hands_.size(); ++i) {
            if (hands_[i]) {
                ...
                std::cout << "\033[36m[Revo1CanTest] INFO: Device " << i << " force_level: " << static_cast<int>(force_level) << "\033[0m" << std::endl;
            ...
            while(running) {
                for (size_t i = 0; i < hands_.size(); ++i) {
                    ...
                    if (hands_[i] && hands_[i]->getFingerStatus(status)) {
                        std::cout << "\033[36m[Revo1CanTest] STATUS: Device " << i << " status: " << status << "\033[0m" << std::endl;
                    }
                }
    ...
    void wave_motion_test() {
        ...
        printf("\033[36m[Revo1CanTest] INFO: 开始位置控制测试 - 波浪运动模式\033[0m\n");

        for(int j = 0; j < 10; ++j) {
            printf("\033[36m[Revo1CanTest] INFO: 第 %d 轮波浪运动\033[0m\n", j + 1);
                ...
                for (size_t dev_idx = 0; dev_idx < hands_.size(); ++dev_idx) {
                    if (hands_[dev_idx]) {
                        ...
                        if(hands_[dev_idx]->getFingerStatus(status)) {
                            printf("\033[36m[Revo1CanTest] STATUS: Device %zu 状态: pos=[%d,%d,%d,%d,%d,%d], cur=[%d,%d,%d,%d,%d,%d]\033[0m\n",
                                dev_idx,
        ...
        printf("\033[32m[Revo1CanTest] INFO: 波浪运动测试完成\033[0m\n");
    }
    ...
    void speed_control_test() {
        ...
        printf("\033[36m[Revo1CanTest] INFO: 测试正向速度...\033[0m\n");
        for (auto& hand : hands_) {
            if (hand) {
        ...
        printf("\033[36m[Revo1CanTest] INFO: 测试反向速度...\033[0m\n");
        for (auto& hand : hands_) {
            if (hand) {
                hand->setFingerSpeeds({-20, -20, -20, -20, -20, -20});
            }
        ...
        printf("\033[32m[Revo1CanTest] INFO: 速度控制测试完成\033[0m\n");
    }
    ...
    void reset_to_initial_state() {
        ...
        printf("\033[32m[Revo1CanTest] INFO: 所有设备已恢复到张开状态\033[0m\n");
    }

    // 运行完整测试
    void run_test() {
        printf("\033[35m=== CAN Revo1灵巧手测试程序 ===\033[0m\n");

        // 初始化CAN总线
        if (!init_can_buses()) {
            printf("\033[31m[Revo1CanTest] ERROR: CAN总线初始化失败，测试终止\033[0m\n");
            return;
        }

        // 连接REVO1设备
        if (!connect_revo1_devices()) {
            printf("\033[31m[Revo1CanTest] ERROR: REVO1设备连接失败，测试终止\033[0m\n");
            return;
        }
...
#ifndef REVO1_CAN_TEST_NO_MAIN
int main() {
    printf("\033[35m[Revo1CanTest] INFO: 启动CAN Revo1灵巧手测试程序\033[0m\n");
    Revo1CanTest test;
    test.run_test();
    return 0;
}
