...
static int init_impl()
{
    ...
    if (pthread_create(&thread_id, NULL, thread_function, NULL) != 0) {
        fprintf(stderr, "[DexterousHand] init Error creating thread, errno:[%d], errstr:[%s]\n", errno, strerror(errno));
        return -errno;
    }
    for (uint8_t i = 0; i < com_num; i++)
    {
        hand_sdk.qiangnao_port[i] = hand_sdk.open_com(&com_list[i]);
        printf("[DexterousHand] init port %d:%s\n", hand_sdk.qiangnao_port[i], com_list[i]);
        printf("[DexterousHand] init Open COM port:%d\n", hand_sdk.qiangnao_port[i]);
    }
    for (uint8_t i = 0; i < com_num; i++)
    {
    ...
    for (uint8_t i = 0; i < com_num; i++)
    {
        ...
        hand_sdk.set_motor_position(hand_sdk.qiangnao_port[i], position);
        // memcpy(hand_sdk.left_value, position, sizeof(position));
    }
    sleep(0.5);

    printf("[DexterousHand] initf ok, inished \n");
    return 0;
}
