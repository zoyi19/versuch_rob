...
void InEKF::Propagate(const Eigen::Matrix<double,6,1>& m, double dt) {
    ...
    Eigen::Vector3d p_d = v*dt + 0.5*(R*a + g_)*dt*dt;
    // if (p_d[2] > 0.0001)
    //     p_d[2] = 0;
    Eigen::Vector3d p_pred = p + p_d;
    // std::cout<<"p_pred-p: "<<p_d.transpose()<<std::endl;
    // std::cout<<"p_pred: "<<p_pred.transpose()<<std::endl;

    // Set new state (bias has constant dynamics)
    state_.setRotation(R_pred);
    state_.setVelocity(v_pred);
    logPublishVector("inekf_state/est/velocity_pred", v_pred);
    state_.setPosition(p_pred);
    state_.setAngularVelocity(w);
