...
该脚本用于测试Roban机器人机械臂的正弦运动跟踪控制，通过控制各个关节的
渐进式正弦变化来实现流畅的机械臂运动效果。测试包含左右臂和头部的
协调运动，用于验证机械臂的位置控制精度和响应性能。

使用方法：
1. 先启动瑞沃电机控制器测试节点：
   ./devel/lib/hardware_node/ruiwo_motor_ros_cxx_test _robot_type:=roban

2. 然后运行此脚本：
   source devel/setup.bash
   python3 roban_arm_tracing_test.py
"""
...
class RobanArmTracingTest:
    def __init__(self):
        rospy.init_node('roban_arm_tracing_test', anonymous=True)
        ...
        self.enable_pub_joint_state = True
        self.rate = rospy.Rate(500)  # 200Hz
        self.joint_names = [
            'zarm_l1_joint', 'zarm_l2_joint', 'zarm_l3_joint', 'zarm_l4_joint',  # Left arm joints (4 joints)
            'zarm_r1_joint', 'zarm_r2_joint', 'zarm_r3_joint', 'zarm_r4_joint',   # Right arm joints (4 joints)
            'zhead_1_joint', 'zhead_2_joint'  # Head joints
        ]
        # Define arm and head joint limits
        self.arm_head_joint_limits = [
            # Left arm joints (4 joints)
            (10, 30),    # zarm_l2
            (30, 60),  # zarm_l3
            (-45, 45),  # zarm_l5
            (-60, 0),    # zarm_l4
            # Right arm joints (4 joints)
            (-30, -10),       # zarm_r2
            (-60, -30),      # zarm_r3
            (-45, 45),    # zarm_r5
            (-60, 0),        # zarm_r4
        ...
        # Phase offsets for arm and head joints (adjusted for 4 joints per arm)
        self.arm_head_phase_offsets = [
            # Left arm joints (4 joints)
            0.0, np.pi/4, np.pi/2, 3*np.pi/4,
            # Right arm joints (opposite phase, 4 joints)
...
if __name__ == '__main__':
    try:
        controller = RobanArmTracingTest()
        controller.run()
