...
########################################## 运动控制 ik求解 #########################################
    ...
    if False:
        ...
        position_flag = -1 if yolo_object_y < 0 else 1  # 右手目标为-1，左手目标为1
    ...
    if args.no_waist==False:
        ...
        exist, target_point, rotate_angle = cal_target_with_rotation(
            a=0.38, b=-1*robot_zero_y, r=-1*robot_zero_y, m=set_x, n=set_y)
        if exist:
    ...
    # 两次ik求解
    rospy.loginfo("第一次ik:")
    res_1st=calculate_ik_result(position_flag, set_x-0.08, set_y+0.06*position_flag, set_z)  # 进行第一次求解
    rospy.loginfo("第二次ik:")
...
########################################## 运动控制 准备姿态 #########################################
        ...
        if  position_flag > 0 :
            # 预抓取姿势
            motion_controller.publish_arm_target_poses([1.5], [70, 0, 10, -120, -10, 0, -35] + ARM_RESET_POSE)#[40, 20, 0, -120, 0, 0, -20]
            rospy.sleep(1.5) 
            ...
            # 预抓取姿势 
            motion_controller.publish_arm_target_poses([1.5], ARM_RESET_POSE + [70, 0, -10, -120, 10, 0, -35])#[40, -20, 0, -120, 0, 0, -20])
            rospy.sleep(1.5) 
...
########################################## 运动控制 执行ik结果 #########################################
        ...
        degrees_list = [math.degrees(rad) for rad in joint_end_angles]
        # 调用函数并传入times和values
        motion_controller.publish_arm_target_poses([1.5], degrees_list)
        rospy.sleep(3.5)
        rospy.loginfo("完成逆解并根据逆解结果到达定位置")
