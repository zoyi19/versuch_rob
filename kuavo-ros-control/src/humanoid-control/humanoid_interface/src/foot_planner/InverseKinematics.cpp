                                       ...
                                       vector3_t des_foot_eular_zyx)
{
  ...
  auto FRAME_ID = info_->endEffectorFrameIndices[leg2point(leg)];
  init_q.segment<6>(6 + index) = computeTranslationIK(init_q, leg, des_foot_linear_xyz, FRAME_ID);
  return computeRotationIK(init_q, leg, des_foot_eular_zyx, FRAME_ID);
}
                                       ...
                                       matrix3_t des_foot_R_des)
{
  ...
  auto FRAME_ID = info_->endEffectorFrameIndices[leg2point(leg)];
  init_q.segment<6>(6 + index) = computeTranslationIK(init_q, leg, des_foot_linear_xyz, FRAME_ID);
  return computeRotationIK(init_q, leg, des_foot_R_des, FRAME_ID);
}

vector6_t InverseKinematics::computeIkWithDesiredFrame(vector_t init_q, int leg, vector3_t des_foot_linear_xyz,
                                                      matrix3_t des_foot_R_des, size_t frameId)
{
  const int index = leg2index(leg);
  init_q.segment<6>(6 + index) = computeTranslationIK(init_q, leg, des_foot_linear_xyz, frameId);
  return computeRotationIK(init_q, leg, des_foot_R_des, frameId);
}

vector6_t InverseKinematics::computeTranslationIK(vector_t init_q, int leg, vector3_t des_foot_linear_xyz, size_t frameId)
{
  ...
  const vector3_t init_foot_pos = data.oMf[frameId].translation();
  ...
  else
  {
    ...
    while (true)
    {
      pinocchio::computeFrameJacobian(model, data, init_q, frameId, pinocchio::LOCAL_WORLD_ALIGNED, Jac);
      Jac_i = Jac.block<3, 6>(0, 6 + index);
      qr_.compute(Jac_i);
      vector6_t v_i = -qr_.solve(err);
      v.segment<6>(6 + index) = v_i;
      ...
      vector3_t cur_foot_pos = data.oMf[frameId].translation();
  ...
  return std::move(init_q.segment<6>(6 + index));
}

vector6_t InverseKinematics::computeRotationIK(vector_t init_q, int leg, vector3_t des_foot_eular_zyx, size_t frameId)
{
  matrix3_t R_des = getRotationMatrixFromZyxEulerAngles(des_foot_eular_zyx);
  return computeRotationIK(init_q, leg, R_des, frameId);
}

vector6_t InverseKinematics::computeRotationIK(vector_t init_q, int leg, matrix3_t des_foot_R_des, size_t frameId)
{
  const scalar_t err_tol = 0.01;
  const scalar_t converage_tol = 0.001;
  const scalar_t damp = 1e-6;
  ...
  else
  {
    ...
    while (true)
    {
      pinocchio::computeFrameJacobian(model, data, init_q, frameId, pinocchio::LOCAL, Jac);
      Jac_i = Jac.block<3, 6>(3, 6 + index);
      Jac_linear_i = Jac.block<3, 6>(0, 6 + index);
