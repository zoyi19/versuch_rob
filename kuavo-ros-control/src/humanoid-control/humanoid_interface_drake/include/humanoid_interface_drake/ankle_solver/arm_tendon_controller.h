...
namespace lower_leg
{
  ...
  class ArmTendonController : public drake::systems::LeafSystem<double>
  {
  public:
    DRAKE_NO_COPY_NO_MOVE_NO_ASSIGN(ArmTendonController)

    ArmTendonController(multibody::MultibodyPlant<double> &plant, const std::vector<std::string> &cfg_str);
    ...
    /*********大迭代版本解算函数维护*********/
    std::pair<double, double> frc_joint2actuator_left(double pitch, double roll, double llbar, double lrbar, double tau_pitch, double tau_roll);
    std::pair<double, double> frc_actuator2joint_left(double pitch, double roll, double llbar, double lrbar, double tau_llbar, double tau_lrbar);
    std::pair<double, double> frc_joint2actuator_right(double pitch, double roll, double llbar, double lrbar, double tau_pitch, double tau_roll);
    std::pair<double, double> frc_actuator2joint_right(double pitch, double roll, double llbar, double lrbar, double tau_llbar, double tau_lrbar);
    /* 膝关节解算 */
    double frc_joint2actuator_knee(double knee, double bar, double tau_knee);
    double frc_actuator2joint_knee(double knee, double bar, double tau_bar);
    /************************************/
