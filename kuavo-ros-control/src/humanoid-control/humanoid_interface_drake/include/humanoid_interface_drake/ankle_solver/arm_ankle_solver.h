...
namespace lower_leg
{
  ...
  class ArmAnkleSolverSystem : public drake::systems::LeafSystem<double>
  {
  public:
    DRAKE_NO_COPY_NO_MOVE_NO_ASSIGN(ArmAnkleSolverSystem)

    ArmAnkleSolverSystem(const drake::multibody::MultibodyPlant<double> &plant,const std::vector<std::string> &cfg_str);

    void CalcQ(VectorXd &q) const;
    void CalcState(Vector4d qv_ankle_arm, VectorXd &ankle_qv);

    /*********大迭代版本解算函数维护*********/
    std::pair<double, double> pos_actuator2joint_left(double llbar, double lrbar);
    std::pair<double, double> vel_actuator2joint_left(double pitch, double roll, double llbar, double lrbar, double llbarDt, double lrbarDt);
    std::pair<double, double> pos_actuator2joint_right(double llbar, double lrbar);
    std::pair<double, double> vel_actuator2joint_right(double pitch, double roll, double llbar, double lrbar, double llbarDt, double lrbarDt);
    /* 膝关节解算 */
    double pos_actuator2joint_knee(double bar);
    double vel_actuator2joint_knee(double knee, double bar, double barDt);
    /************************************/
