...
class ContinuousStairClimber:
    ...
    def plan_forward_movement(self, distance=0.2):
        ...
        time_traj, foot_idx_traj, foot_traj, torso_traj, swing_trajectories = self.plan_move_to_4d(
            ...
            max_step_x=0.15, max_step_y=0.15, max_step_yaw=30.0
        )
...
def play_tai_chi():
    # 执行一个python命令
    os.system("python3 ./src/demo/csv2body_demo/test_step_player_csv_ocs2.py")

# 转头
# 转头
def turn_head(target_position=[0,0]):
    # 创建头部运动发布器
    head_motion_pub = rospy.Publisher("/robot_head_motion_data", robotHeadMotionData, queue_size=10)
    ...
    for i in range(steps + 1):
        ...
        msg.joint_data = interpolated_position
        head_motion_pub.publish(msg)
