...
# 启动 stress-ng 对特定核心进行压力测试 (原 run_stress_237.sh 功能)
STRESS_NG_CPUS=(2 3 7 14 15)
echo "启动 stress-ng 对特定核心进行压力测试: ${STRESS_NG_CPUS[*]}" | tee -a "$LOG_FILE"
for cpu in "${STRESS_NG_CPUS[@]}"; do
    stress-ng --cpu 1 --taskset "$cpu" --timeout "${TEST_DURATION}s" >/dev/null 2>&1 &
done
