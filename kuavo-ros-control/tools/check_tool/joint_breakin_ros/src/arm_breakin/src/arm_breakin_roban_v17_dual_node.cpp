...
public:
    ArmBreakinNode(int argc, char** argv) : nh_(nullptr), arm_ready_(false), arm_running_(false), start_received_(false), 
        ...
        ros::init(argc, argv, "arm_breakin_roban_v17_dual_node");
        ...
        if (action_config_file_path_.empty()) {
            ...
            if (len != -1) {
                ...
                if (pos1 != std::string::npos) {
                    ...
                    search_paths.push_back(join_path(workspace_root, {"config", "arm_breakin", "arm_breakin_roban_v17_dual_config.yaml"}));
                    ...
                    for (int i = 0; i < 10; ++i) {  // 最多向上10级
                        std::string test_path = join_path(current_path, {"config", "arm_breakin", "arm_breakin_roban_v17_dual_config.yaml"});
            ...
            if (ros_workspace != nullptr) {
                search_paths.push_back(join_path(ros_workspace, {"config", "arm_breakin", "arm_breakin_roban_v17_dual_config.yaml"}));
            }
            ...
            if (getcwd(cwd, sizeof(cwd)) != nullptr) {
                ...
                search_paths.push_back(join_path(workspace_path, {"config", "arm_breakin", "arm_breakin_roban_v17_dual_config.yaml"}));
                search_paths.push_back(join_path(workspace_path, {"..", "config", "arm_breakin", "arm_breakin_roban_v17_dual_config.yaml"}));
                // 如果在src目录下
