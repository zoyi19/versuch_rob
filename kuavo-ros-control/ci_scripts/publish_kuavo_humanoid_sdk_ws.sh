...
SDK_PROJECT_DIR="$PROJECT_DIR/src/kuavo_humanoid_websocket_sdk"  # project: kuavo_humanoid_websocket_sdk
...
notify_to_wxwork_group() {
    local msg=$1
    if [ -z "$msg" ]; then
        echo "[kuavo-humanoid-sdk-ws] No message provided for notification"
        return 1
    fi
...
exit_with_failure() {
    ...
    if [ -n "$1" ]; then
        ...
        notify_msg="[Kuavo Humanoid WebSocket SDK] 发布失败, 错误信息: $1"
...
exit_with_success() {
    ...
    msg="[Kuavo Humanoid SDK] 🎉 最新$version($VERSION)已经发布 🚀: https://pypi.org/project/kuavo-humanoid-sdk-ws/$VERSION"
...
exit_with_cancel() {
    ...
    clean_cache
    echo -e "\033[33m[kuavo-humanoid-sdk-ws]取消发布, 原因是: $msg\033[0m"
    exit 0
}
...
echo -e "\033[32m[kuavo-humanoid-sdk-ws] Current version: $VERSION\033[0m"
echo -e "\033[32m[kuavo-humanoid-sdk-ws] Current branch: $BRANCH\033[0m"
install_dependencies
clean_cache     
# copy_deps_msgs  
build_package   
