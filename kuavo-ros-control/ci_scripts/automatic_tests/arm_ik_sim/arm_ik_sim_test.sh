...
RUN_DISPLAY=:1
RUN_PREFIX=""
RUN_LOG_DIR="$auto_test_arm_ik_log_path"
DEBUG_MODE=0
# debug mode
if [ $DEBUG_MODE -eq 1 ]; then
    RUN_PREFIX=""
    RUN_LOG_DIR="/tmp/ci_arm_ik_sim"
fi
if [ -z "$RUN_LOG_DIR" ]; then
    echo "RUN_LOG_DIR is not set"
    echo "❌ miss env which is named auto_test_arm_ik_log_path"
    # RUN_LOG_DIR="/tmp/ci_arm_ik_sim"
    exit 1
fi
...
modify_config() {
    ...
    # 拷贝 ik_pose_samples.jsonl (强制覆盖)
    ik_pose_samples_file="$PROJECT_DIR/src/automatic_test/automatic_test/scripts/half_body_API_test/actions/ik_pose_samples.jsonl"
    # 确保目标目录存在
    sudo mkdir -p "$(dirname "$ik_pose_samples_file")"
    # 直接覆盖文件
    sudo cp "$SCRIPT_DIR/ik_pose_samples.jsonl" "$ik_pose_samples_file"
    echo_success "✅ ik_pose_samples.jsonl copied successfully (overwritten)"
}
...
launch_humanoid_controllers() {
    ...
    sudo tmux new-session -d -s $HUMANOID_CONTROLLERS_SCREEN_NAME bash -c "
        ...
        $RUN_PREFIX roslaunch humanoid_controllers load_kuavo_mujoco_sim.launch coredump:=true | tee /tmp/$HUMANOID_CONTROLLERS_SCREEN_NAME.log
    " 
    sleep 5
}
...
launch_test() {
    ...
    source /opt/ros/noetic/setup.bash
    source devel/setup.bash
    # ---------- Python 参数集中设置（可通过环境变量覆盖） ----------
    IK_FILE_DEFAULT="$PROJECT_DIR/src/automatic_test/automatic_test/scripts/half_body_API_test/actions/ik_pose_samples.jsonl"
    IK_FILE=${IK_FILE:-$IK_FILE_DEFAULT}
    IK_RATIO=${IK_RATIO:-0.5} # 样本总数 1000，抽取比例 0.5
    IK_OUT=${IK_OUT:-/tmp/ik_pose_replay_results.jsonl}
    IK_SLEEP=${IK_SLEEP:-10.0} # 每次手臂运动
    IK_WAIT_TIME=${IK_WAIT_TIME:-4.0} # 在总等待时间中预留的收敛/评估时间（秒）
    BASE_FRAME=${BASE_FRAME:-base_link}
    RIGHT_EE_FRAME=${RIGHT_EE_FRAME:-zarm_r7_end_effector}
    LEFT_EE_FRAME=${LEFT_EE_FRAME:-zarm_l7_end_effector}
    POS_ERR_THRESH=${POS_ERR_THRESH:-0.10} # 位置误差阈值
    ORI_ERR_THRESH_DEG=${ORI_ERR_THRESH_DEG:-10.0} # 姿态误差阈值
    IK_REQ_RATE=${IK_REQ_RATE:-0.90} # ik 解算的成功率
    REACH_REQ_RATE=${REACH_REQ_RATE:-0.90} # ik 解算成功手臂运动到指定位置的成功率
    CHECK_ORI=${CHECK_ORI:-0}  # 0: 只比较位置; 1: 同时比较姿态（目前姿态误差较大，在 15 度以上）
    if [ "$CHECK_ORI" = "1" ]; then
        CHECK_ORI_OPT="--check_ori"
    else
        CHECK_ORI_OPT=""
    fi

    echo_info "运行参数: --file=$IK_FILE --ratio=$IK_RATIO --out=$IK_OUT --sleep=$IK_SLEEP --wait_time=$IK_WAIT_TIME"
    echo_info "           --base_frame=$BASE_FRAME --right_ee_frame=$RIGHT_EE_FRAME --left_ee_frame=$LEFT_EE_FRAME"
    echo_info "           --pos_err_thresh=$POS_ERR_THRESH --ori_err_thresh_deg=$ORI_ERR_THRESH_DEG $CHECK_ORI_OPT"
    echo_info "           --ik_req_rate=$IK_REQ_RATE --reach_req_rate=$REACH_REQ_RATE"
    ...
    rosrun automatic_test replay_ik_pose.py \
        ...
        --out "$IK_OUT" \
        --sleep "$IK_SLEEP" \
        --wait_time "$IK_WAIT_TIME" \
        --base_frame "$BASE_FRAME" \
        --right_ee_frame "$RIGHT_EE_FRAME" \
        --left_ee_frame "$LEFT_EE_FRAME" \
        --pos_err_thresh "$POS_ERR_THRESH" \
        --ori_err_thresh_deg "$ORI_ERR_THRESH_DEG" \
        $CHECK_ORI_OPT \
        --ik_req_rate "$IK_REQ_RATE" \
        --reach_req_rate "$REACH_REQ_RATE"
    sleep 5
    check_results /tmp/ik_pose_replay_results.jsonl
    if [[ $? -eq 0 ]]; then
        echo_success "✅ success "
    else
        exit_with_failure "❌ IK 轨迹跟踪失败"
    fi
...
check_robot_launch() {
    ...
    for i in {1..5}; do
        ...
        if rosnode list | grep -q "/humanoid_sqp_mpc"; then
            echo_success "✅ Found required nodes: /humanoid_sqp_mpc"
            humanoid_controllers_flag=1
        else
            echo_error "❌ Missing required nodes: /humanoid_sqp_mpc"
...
check_results() {
    local file=${1:-/tmp/ik_pose_replay_results.jsonl}
    ...
    if [[ -n "$RUN_LOG_DIR" ]]; then
        mkdir -p "$RUN_LOG_DIR"
        sudo cp "$file" "$RUN_LOG_DIR/ik_pose_replay_results.jsonl"
        echo_success "✅ 结果文件已复制到: $RUN_LOG_DIR/ik_pose_replay_results.jsonl"
    else
        echo_warn "⚠️ RUN_LOG_DIR 未设置，跳过结果文件复制"
    fi

    # 从 JSONL 首行读取 summary.test_pass 判定 CI 成败（使用 jq）
    local success=$(head -n 1 "$file" | jq -r '.summary.test_pass // false' 2>/dev/null)

    # 展示结果
    if [[ "$success" == "true" ]]; then
        echo -e "\033[32m================== RESULT SUMMARY (SUCCESS) ==================\033[0m"
        head -n 1 "$file" | sed $'s/^/\033[32m/; s/$/\033[0m/'
        echo_success "✅ 测试成功"
        return 0
    else
        echo -e "\033[31m================== RESULT SUMMARY (FAILED) ===================\033[0m"
        head -n 1 "$file" | sed $'s/^/\033[31m/; s/$/\033[0m/'
        echo_error "❌ 测试失败"
        return 1
    fi
}
